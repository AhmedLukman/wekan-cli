...
async fn yes_bypasses_the_confirmation_provider() {
    let store = FakeCredentialStore::local_only(true);
    let confirmation = FakeConfirmationProvider::failing(crate::error::AppError::invalid_input(
        "must not be called",
    ));
...
async fn local_only_named_logout_deletes_only_that_profile_without_network_http_opt_in() {
    ...
    let selected_profile = FakeCredentialStore::local_only_account(Some("profile:test-store:work"));
...
async fn expired_credentials_are_still_revoked_and_deleted() {
    ...
    let store =
        FakeCredentialStore::with_record(&server, OffsetDateTime::now_utc() - Duration::days(1));
